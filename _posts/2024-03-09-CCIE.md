...
接口模式(设置nat内部和外部接口,所有NAT都需要):
  ...
  - ip nat outside 
- 静态NAT
   - ip nat inside source static `inside-local` [`port`] `inside-golbal` [`port`] [extendable]
...
- VSS(Virtual Switching System),SWV(StackWise Virtual),clustering two physical switches into a single logical switch(distribution or core layer often)
...
2. tunnel IPsec profile
   1. crypto isakmp policy `priority`(!1 is the highest)
      ...
      3. authentication {rsa-sig|rsa-encr|pre-share}
      4. group {1|2|5|14|15|16|19|20|24}(!recommend to use 14 or higher)
   2. crypto isakmp key `keystring` address `address` [mask]
   ...
   5. 进入待加密gre接口
      1. tunnel protection ipsec profile `profilename` [shared]
...
1. IPsec配置文件:将转换集和IKEv2配置文件关联起来.
...
3. tacacs-server host {`server-name`|`ip`} key `key`(!for prior version)
4. tacacs server `name`
   1. address ipv4 {`hostname`|`ip`}
   2. key `key`
5. radius server `server-name`
6. aaa group server {radius|tacacs+} `group-name`
   1. server name `server-name`(!allow nultiple servers)
