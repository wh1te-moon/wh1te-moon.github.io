...
- LLQ
  CBWFQ with PQ,more suitble for real-time traffic.
...
2. policy-map `policy-map-name`
   1. class `class-name` 
      1. {priority|bandwidth|service-polocy|set 
      qos-group `qos-group-id`
      |cos `cos-value`
      |[ip] precedence `ipp`
      |[ip] dscp `dscp-value`
      }
   2. policy [cir] `cir-in-bps` [[bc] `Bc`] [[be] `Be`(!default equeals Bc)] [pir `Pir`] [comform-action `action`(!default transmit)] [exceed-action `action`(!default drop)] [violate-action `action`(!default drop)] (!cir,bc,be are just nonates)
