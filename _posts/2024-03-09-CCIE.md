...
#### 4. DUAL
自身对某一特定子网的度量值叫做FD(可行距离),而可以到达该子网的邻居报告的其自身的FD称为RD,当RD小于FD时,报告RD的邻居即为该子网的feasible successor.当没有feasible successor时,会通过DUAL(弥散更新算法)查询那些不是feasible successor的邻居是否可行.feasible successor中FD最小的为successor,即装入路由表中中的路由.
...
6. (!可选)standby `instance-id` priority `num`(!higher priority is preferd,then higher ip)
7. (!可选)standby `instance-id` name `name`
...
- wave
  ...
  - RF Power
    ...
    - the power level of received signal should be greater than the **sensitivity level** of receiver.
      - be greater than the noise level
      - SNR(signal-to noise ratio)(db):higher is prefered
  - modulation
    - frequency
    - phase
    - amptilude
...
- Lightweight:has to join a WLC(wireless LAN controller) to become fully functional.(cooperation named split-MAC architecture).An AP and a WLC are joined by a logical pair of CAPWAP tunnels.(every WLC can support maximum number of APs in use)
  - modes:
    - Local:offer BSSs
    - FlexConnect:Forward data normally with a remote WLC.
    - Monitor:Its receiver acts as a dedicated sensor.
    - Sniffer:Receive 802.11 traffic from other sources,forwarding to a PC
    - Rouge detector
...
- WLC discover to build a list of candidates
  ...
  3. DHCP server suggest a list of WLC addresses
...
3. EAP with 802.1x(based on WPA enterprise mode)(should always use as higher as possible version)
802.1x consists of
   - supplicant:client
     - - must specify the EAP method
   - authentication:AP and WLC
...
2. Fabric border node:PxTRs,connects external Layer 3 device to the SDA fabric
   ...
   and one function:
   4. translate VRF and SGT infomation from one domain to another
...
1. cisco SD-WAN
   ...
   2. vManage Network Management System(NMS):provides GUI for managing the SD-WAN solution
...
use VTEPs(virtual tunnel endpoints) to discover VNI over the underlay Layer 3.Each VTEP has two parts:
...
5. Cisco Umbrella
6. Cisco WSA(secure web application)
...
13. Cisco ISE(indentity services engine)
