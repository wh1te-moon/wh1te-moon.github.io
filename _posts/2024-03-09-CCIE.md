...
配置模式:
1. aaa new-model
2. username `username` password  `num` `password`
3. tacacs server `server-name`
4. radius server `server-name`
5. address ipv4 `address`
6. key `key`
7. port `port`
8. aaa authentication default `method1` `method2`(!medthod={`group-name|server-name`|local(!本地用户名认证)|login(!密码认证)})
...
AD值高的协议重分发到AD低的协议时,如果存在两个以上边界路由器,要小心流量黑洞(例如EIGRP->OSPF或iBGP作为源),可以使用distance命令设置AD值.
...
配置模式:
1. ip vrf `name`
   1. interface `interface`(!如果是子接口,必须是dot1q封装方式)
